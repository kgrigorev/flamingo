...
func RootCommandProvider(
	...
	preRun := func(cmd *cobra.Command, _ []string) error { //nolint:unparam // to fit RunE signature
	...
	postRun := func(cmd *cobra.Command, _ []string) error {
		// on finished command execution
		// stop listening for signals by calling stop()
		// wait for context to be cancelled (should happen immediately)
